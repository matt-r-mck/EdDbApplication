...
namespace EdDbApplication {
    class Program {
        ...
        static void Test1() {
            ...
            var sqlConnection = new SqlConnection(connectionString);//instance of sql, passing connection string
            sqlConnection.Open();//call open on that constructor
            if (sqlConnection.State != ConnectionState.Open) {
                throw new Exception("Connection did not open");//in case connection fails
            }

            var sql = "Select * from Student;";//sql select statement
            var sqlCmd = new SqlCommand(sql, sqlConnection);//into sql command that needs a statement and connection on which to operate
            var reader = sqlCmd.ExecuteReader();//starts reader

            var students = new List<Student>(100);//initalizes new list of up to 100 students

            while (reader.Read()) {
                var id = Convert.ToInt32(reader["Id"]); //looks for id, passes into variable
                var student = new Student(id); // empty instance of one student passing ID from database
                student.Firstname = reader["FirstName"].ToString(); //sets results from read to values of properties for each student instance during while loop
            ...
            reader.Close();//stops the reader
            sqlConnection.Close();//closes the cocnnection
        }
    }
}